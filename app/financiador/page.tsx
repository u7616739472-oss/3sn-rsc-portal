...
import { Wallet, Users, BadgeCheck, ArrowRight } from 'lucide-react'
...
const financiadores = [
  { id: 1, nombre: 'Fundación Aurora', descripcion: 'Convocatorias para educación y desarrollo digital.', tipo: 'Fundación', image: 'https://images.unsplash.com/photo-1554224155-1696413565d3?q=80&w=1200&auto=format&fit=crop' },
  { id: 2, nombre: 'Banco Solidario', descripcion: 'Líneas de crédito y grants para impacto social.', tipo: 'Banca', image: 'https://images.unsplash.com/photo-1563013544-824ae1b704d3?q=80&w=1200&auto=format&fit=crop' },
  { id: 3, nombre: 'Agencia Global', descripcion: 'Cooperación internacional para salud y clima.', tipo: 'Agencia', image: 'https://images.unsplash.com/photo-1542228262-3d663b306a56?q=80&w=1200&auto=format&fit=crop' },
]
  ...
  return (
    <Layout>
      <section className="mb-8 flex items-center justify-between">
        <h1 className="text-2xl md:text-3xl font-semibold tracking-tight text-white">Financiadores</h1>
      </section>

      <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {financiadores.map((f) => (
          <article key={f.id} className="group relative overflow-hidden rounded-2xl border border-white/10 bg-white/5 backdrop-blur-md transition hover:border-white/20 hover:bg-white/10">
            <div className="relative h-40 w-full overflow-hidden">
              <Image src={f.image} alt={f.nombre} fill className="object-cover transition duration-500 group-hover:scale-105" />
            ...
            <div className="p-5">
              <div className="mb-3 flex items-center gap-2">
                ...
                <span className="rounded-full bg-orange-500/20 px-2 py-0.5 text-xs text-orange-300">{f.tipo}</span>
              </div>
              <h3 className="text-lg font-semibold text-orange-400">{f.nombre}</h3>
              <p className="mt-2 text-sm text-white/80">{f.descripcion}</p>
              <div className="mt-4">
